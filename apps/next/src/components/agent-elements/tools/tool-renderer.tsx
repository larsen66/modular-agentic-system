...
export const ToolRenderer = memo(function ToolRenderer({
  ...
  switch (partType) {
    ...
    case "tool-Thinking": {
      // Auto-expand while reasoning streams, then COLLAPSE once done. `defaultOpen` is uncontrolled,
      // so it only takes effect on mount — keying on the streaming state remounts the row when thinking
      // finishes, re-initializing it closed (the user can still click to reopen).
      const live = part.state === "input-streaming";
      return <ThinkingTool key={live ? "thinking-live" : "thinking-done"} part={part} defaultOpen={live} />;
    }
  }
