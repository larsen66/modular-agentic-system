...
export type InputBarProps = {
  ...
  infoBar?: {
    title?: string;
    description?: string;
    /** Custom React node rendered in place of description when provided. */
    descriptionNode?: React.ReactNode;
    onClose?: () => void;
    position?: "top" | "bottom";
    /** Replace the default X icon in the close button with a custom icon node. */
    closeIcon?: React.ReactNode;
    /** When true, clicking close only fires onClose — does not hide the bar (use for toggles). */
    persistent?: boolean;
...
export const InputBar = memo(function InputBar({
  ...
  const handleInfoBarClose = useCallback(() => {
    if (!infoBar?.persistent) setIsInfoBarOpen(false);
    infoBar?.onClose?.();
  }, [infoBar]);

  const infoBarPosition = infoBar?.position ?? "top";
  const shouldShowInfoBar = Boolean(
    infoBar && (infoBar.title || infoBar.description || infoBar.descriptionNode || infoBar.onClose),
  );
  ...
  const infoBarNode = shouldShowInfoBar ? (
    <div
      className={cn(
        ...
        infoBarData.closeIcon
          ? "rounded-an-input-border-radius"
          : infoBarPosition === "top"
            ? "rounded-t-an-input-border-radius"
            : "rounded-b-an-input-border-radius",
      )}
    >
      <div className="min-w-0 flex items-center gap-1.5 text-xs text-an-foreground overflow-hidden">
        {infoBarData.title && (
          <span className="font-medium shrink-0">{infoBarData.title}</span>
        )}
        {infoBarData.descriptionNode ? (
          <span className="min-w-0 truncate text-an-foreground-muted/80">{infoBarData.descriptionNode}</span>
        ) : infoBarData.description ? (
          <span className="min-w-0 truncate text-an-foreground-muted/80">
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {infoBarData.onClose && (
          <button
            ...
            aria-label="Close"
          >
            {infoBarData.closeIcon ?? <IconX className="w-3.5 h-3.5" strokeWidth={2} />}
          </button>
        )}
      </div>
    </div>
                ...
                <>
                  <textarea
                    ...
                    className={cn(
                      ...
                      "[field-sizing:content] overflow-x-hidden overflow-y-auto",
                      disabled && "opacity-50 cursor-not-allowed",
                    )}
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    style={{ maxHeight: 120, ...({ fieldSizing: 'content' } as any) }}
                  />
